...
module.exports = {
	...
	async execute(client, interaction) {
        ...
        await getRandomMeme("penis", function (memes) {
            ...
            if (!memes || memes.length === 0) {
                ...
                interaction.editReply({embeds: [new Discord.MessageEmbed().setURL(curMeme.url).setImage(curMeme.imageUrl).setColor(Utils.EmbedColors.Success).setTitle(curMeme.title).setFooter(`👍 ${curMeme.ups} | 💬 ${curMeme.comments} (${"penis"})`,).setTimestamp(new Date())]})
            }
        })
